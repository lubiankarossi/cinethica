...
typedef struct {
 ...
 double al  [MAX_NGR], /* Lambda dos precursores */
        beta[MAX_NGR]; /* Betas dos precursor    */
 ...
 int nn     , /* Número de pontos para o cálculo da distribuição */
     iflux  , /* Flag do fluxo de refrigerante.                  */
...
//       printf("%s - %s - %i \n\n",section,name,pdata->nn);
    ...
    else if (MATCH("parameter"         , "iflux" )) {   
//       printf("%s - %s - %s \n",section,name,value);
       pdata->iflux = atoi(value);
//       printf("%s - %s - %i \n\n",section,name,pdata->iflux);
    }
